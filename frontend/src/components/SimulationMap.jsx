...
export default function SimulationMap({ zones = [], selectedZone, activeDisasters = [] }) {
    ...
    // ✅ Prevent infinite re-renders by memoizing disasters
    const stableDisasters = useMemo(() => {
        return Array.isArray(activeDisasters) ? [...activeDisasters] : []
    }, [activeDisasters.length])

    useEffect(() => {
        if (!Array.isArray(stableDisasters)) return

        const newEffects = stableDisasters.map(disaster => {
    ...
    return (
        <div className="relative w-full h-96 rounded-xl border-2 border-gray-300 overflow-hidden shadow-lg">
            ...
            {/* Zones */}
            {zones.map(zone => {
                const hasActiveDisaster = stableDisasters.some(d => d.zoneId === zone.id)
            ...
            <span className="absolute -top-8 left-2 text-sm font-bold text-white bg-gray-800 bg-opacity-80 px-3 py-1 rounded-full border border-white border-opacity-30 shadow-lg">
              {zone.name}
                        ...
                        {/* Live Network Info Overlay */}
                        {zone.metrics && (
                            <div className="absolute inset-0 flex flex-col justify-center items-center text-xs font-medium text-white bg-black bg-opacity-40 rounded-lg backdrop-blur-sm">
                                <div className={`font-bold ${getStatusColor(zone.status)}`}>
                                    {zone.status || 'OK'}
                                </div>
                                <div className="mt-1 text-center space-y-0.5">
                                    <p>Latency: {zone.metrics.latency.toFixed(1)} ms</p>
                                    <p>↓ {zone.metrics.download.toFixed(1)} Mbps</p>
                                    <p>↑ {zone.metrics.upload.toFixed(1)} Mbps</p>
                                    <p>Loss: {zone.metrics.packetLoss.toFixed(1)}%</p>
                                    <p>Risk: {zone.metrics.predictedOutageRisk.toFixed(1)}</p>
                                </div>
                            </div>
                        )}
                    </div>
            ...
            {stableDisasters.length > 0 && (
                <div className="absolute top-4 right-4 bg-red-500 text-white px-3 py-1 rounded-full text-sm font-bold animate-pulse shadow-lg">
                    🚨 {stableDisasters.length} Active Disaster
                    {stableDisasters.length > 1 ? 's' : ''}
                </div>
            )}
        </div>
    )
